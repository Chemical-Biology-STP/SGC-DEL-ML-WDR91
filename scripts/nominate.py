...
michael_acceptor_smarts = [Chem.MolFromSmarts(s) for s in michael_acceptor_smarts]
...
hits = preds[preds["SCORE"] >= 0.6]

hits.sort_values(by='SCORE', inplace=True, ascending=False)

filter = DrugDesignFilters()
filter_res = pd.DataFrame(filter.filter(hits["SMILES"].tolist()), index=hits.index)
hits_filtered = pd.merge(hits, filter_res, left_index=True, right_index=True)

fps = [AllChem.GetMorganFingerprintAsBitVect(Chem.MolFromSmiles(smi), radius=3, nBits=2048) for smi in tqdm(hits_filtered["SMILES"])]
...
hits_filtered.sort_values(["medchem_score", "SCORE"], ascending=False, inplace=True)
...
top_50_picks = hits.sort_values(by='SCORE', inplace=True, ascending=False).iloc[:50]
